...
    
    if (difficulty >= 10) return 'Extreme Demon'; 
...
        
        // 2. Merge Records from VICTOR_COMPLETIONS
...
            minWR: level.minWR || 0,
            difficultyEst: mapDifficulty(level.difficultyEst || 'N/A') 
        };
    });
...
        handleLevelChange(); // Run once on setup
    }
}
...
        // Data processing is now handled by initializeApp() on DOMContentLoaded.
        sidebar.innerHTML += '<p style="padding: 10px;">Levels failed to load. Check console for LEVEL_DATA errors.</p>';
        return;
    }
...
// Setup function (only resets the output on page load)
function setupStatsViewer() {
...
    }
    if (outputElement) {
        outputElement.innerHTML = '<p style="text-align: center; color: #aaa; margin-top: 20px;">Search for a player to see their rank, points, and completions.</p>';
    }
}

// Core search/render function
...
    // Use the pre-calculated leaderboard data
...
            </div>
            <div class="stat-box">
                <div class="value">#${player.hardestRank} (${player.hardestLevel})</div>
...
/**
 * Main initialization function to process data and set up page elements.
 */
function initializeApp() {
    // 1. Process the data first (Calculates points/ranking/WRs)
    processLevelData();

    // 2. Setup Page Specific Logic (Event Listeners, Select Options, etc.)
    setupSubmitPage();
    setupStatsViewer();
}
